...
export const BASE_TOKENS_FARM_MAP = {
  [CHAIN_ID.BNB_TEST_NET]: {
    [ethers.utils.getAddress('0x0D7747F1686d67824dc5a299AAc09F438dD6aef2')]: [
      ethers.utils.getAddress(getWETHAddress(CHAIN_ID.BNB_TEST_NET)),
    ],
...
export const USER_FARM_DATA_PRICES = {
  [CHAIN_ID.BNB_TEST_NET]: {
    makeBaseTokensPriceMap(
      pairAddress: string,
      prices: ReadonlyArray<BigNumber>
    ) {
      return BASE_TOKENS_FARM_MAP[CHAIN_ID.BNB_TEST_NET][
        ethers.utils.getAddress(pairAddress)
      ].reduce(
        (acc, value, index) => ({ ...acc, [value]: prices[index] }),
        {} as { [key: string]: BigNumber }
      );
    },
  },
...
export const WBNB_INT_ADDRESS_MAP = {
